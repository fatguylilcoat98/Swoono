...
export default function TugOfWarGame({
  ...
  console.log('TugOfWar: Starting with roomCode:', roomCode, 'selfClientId:', selfClientId);
  ...
  try {
    ...
    console.error('TugOfWar: Component error:', error);
    return (
      <div className="flex-1 flex flex-col items-center justify-center">
        <div className="text-red-400 mb-4">Error loading Tug of War</div>
        <div className="text-swoono-dim text-sm">{error instanceof Error ? error.message : 'Unknown error'}</div>
        <button
          onClick={onExit}
          className="mt-4 px-4 py-2 bg-swoono-accent text-black rounded"
        >
          Exit Game
        </button>
      </div>
